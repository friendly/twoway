...
[twoway.sas](http://www.datavis.ca/sasmac/twoway.md).
...
allows for a robust fitting by row and column medians, using Tukey’s
idea of median polish, as implemented in
[`stats::medpolish()`](https://rdrr.io/r/stats/medpolish.html). This
uses `method="median"` in the call to
...
![](reference/figures/README-ex1-plot-1.png)
...
![](reference/figures/README-ex1-plot2-1.png)
...
[`anova()`](https://rdrr.io/r/stats/anova.html) method for a `"twoway"`
object gives the results of two analyses: one for the additive model,
and one for the model allowing 1 df for non-additivity. Both of these
assume that row and column effects are fit using means
(`method="mean"`).
...
- The [`as.data.frame()`](https://rdrr.io/r/base/as.data.frame.html)
  method for a `"twoway"` object gives a tidy data.frame result,
...
- The utility functions
  ...
  facilitate conversion between matrix format and a long format with
  variables such as `Row`, `Col` and `Value`.
